...
1. **Self-reference.** A hash *of the file* cannot live *in the file*: adding
   the field changes the bytes, which changes the hash. You would have to define
   a canonicalisation ("hash everything except this field"), which is exactly the
   fragile, easy-to-get-subtly-wrong machinery a self-describing JSON artifact
   should avoid.
2. **It would break the strict clients.** The canonical schema is
   `additionalProperties: false` at every level, and the rosetta-frida (Zod) and
   rosetta-xposed (Kotlin) clients track it strictly. A **new field that real
   maps carry** would be rejected by every client that has not yet shipped the
   matching schema bump — a breaking change for a guarantee that does not need to
